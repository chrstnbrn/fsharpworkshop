...
[<Literal>]
let JsonExample = __SOURCE_DIRECTORY__ + "/Data.json"

type PurchaseData = JsonProvider<JsonExample>

let getPurchases customer =
    let purchaseAverage =
        PurchaseData.Load "Data.json"
        |> Seq.filter (fun c -> c.CustomerId = customer.Id)
        |> Seq.collect (fun c -> c.PurchasesByMonth)
        |> Seq.average

    (customer, purchaseAverage)

let increaseCredit condition customer =
    if condition customer then
        { customer with
              Credit = customer.Credit + 100M<USD> }
    else
        { customer with
              Credit = customer.Credit + 50M<USD> }
    ...
    let customers =
        [ { Id = 1
          ...
          { Id = 2
            ...
            Notifications = ReceiveNotifications(true, true) }
          { Id = 3
            IsVip = false
            Credit = 30m<USD>
            PersonalDetails =
            ...
            Notifications = ReceiveNotifications(true, false) }
          { Id = 4
            IsVip = true
            Credit = 50m<USD>
            PersonalDetails =
                Some
                    { FirstName = "Joe"
                      LastName = "Smith"
                      DateOfBirth = DateTime(1986, 3, 3) }
            Notifications = ReceiveNotifications(false, true) } ]

    customers |> List.find (fun c -> c.Id = id)
